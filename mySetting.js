...
export default function mySetting({navigation, route}) {
  ...
  async function updateAccount(text, id) {
    ...
    if (validateInputs(text)) {
      ...
      switch (modalName) {
        case "acc_name":
          update = {fields: {acc_name: text}};
          break;
        case "acc_pw":{}          
            update = {fields: {acc_pw: text}};
            ToastAndroid.show("密碼已修改!", ToastAndroid.LONG);
            break;     
        case "acc_phone":
  ...
  function Name() {
    return (
      <TextInput
        ...
        placeholder={"姓名"}
        defaultValue={accountData.acc_name} 
  ...
  function Password() {
    return (
        <TextInput 
            ...
            onEndEditing={t => {checkPW(t.nativeEvent.text)}}
        />
    )
  }
      ...
      <ScrollView style={{flex: 1, height: height}} contentContainerStyle={styles.detailContainer}>
        ...
        <View style={styles.formGroup}>
          <TouchableOpacity
            ...
            onPress={() => {
              setModalVisible(true),
              setModalName("acc_name")
            }}>
            <Text style={styles.formLabel}>姓名</Text>
            <Text style={styles.formValue}>
              {(accountData.acc_name !== undefined ? accountData.acc_name: "請輸入...")+' 〉'}
            </Text>
          </TouchableOpacity>
